...
class DataSourceFactory(BaseFactory, ParentExtractMixin):
    ...
    async def load_from_title(self, data_source_title: str, min_similarity: float = 0.6) -> NotionDataSource:
        ...
        potential_data_sources = await self._extract_potential_data_sources_from_search_result(search_response)

        best_match = find_best_match(
            query=data_source_title,
            items=potential_data_sources,
            text_extractor=lambda data_source: data_source.title,
            min_similarity=min_similarity,
        )

        if not best_match:
            available_titles = [result.title for result in potential_data_sources[:5]]
            raise ValueError(
    ...
    async def _create_from_response(self, data_source_dto: DataSourceDto) -> NotionDataSource:
        ...
        title = await self._extract_title(data_source_dto)
        description = await self._extract_description(data_source_dto)
        parent_database = await self._extract_parent_database(data_source_dto)
