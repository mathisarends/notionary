...
def create_renderer_chain() -> BlockRenderer:
    ...
    (
        toggle_handler.next(heading_handler)
        .next(callout_handler)
        .next(code_handler)
        .next(quote_handler)
        .next(todo_handler)
        .next(bulleted_list_handler)
        .next(divider_handler)
        .next(column_list_handler)
        .next(column_handler)
        .next(numbered_list_handler)
        .next(bookmark_handler)
        .next(image_handler)
        .next(video_handler)
        .next(audio_handler)
        .next(file_handler)
        .next(pdf_handler)
        .next(embed_handler)
        .next(equation_handler)
        .next(table_of_contents_handler)
        .next(breadcrumb_handler)
        .next(table_handler)
        .next(table_row_handler)
        .next(paragraph_handler)
