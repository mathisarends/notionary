...
class ImageParser(LineParser):
    """Handles image blocks with [image](url) syntax."""
    ...
    async def _process(self, context: BlockParsingContext) -> None:
        block = await self._create_image_block(context.line)
        if block:
            context.result_blocks.append(block)

    async def _create_image_block(self, text: str) -> CreateImageBlock | None:
        """Create an image block from markdown text."""
        image_path = self._extract_image_path(text.strip())
        if not image_path:
            return None
        ...
        return CreateImageBlock(image=image_data)
    ...
    def _extract_image_path(self, text: str) -> str | None:
        ...
        match = self.IMAGE_PATTERN.search(clean_text)
