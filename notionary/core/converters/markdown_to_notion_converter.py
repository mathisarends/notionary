...
class MarkdownToNotionConverter:
    ...
    def convert(self, markdown_text: str) -> List[Dict[str, Any]]:
        ...
        processed_text, multiline_blocks = self._process_multiline_elements(
            processed_text
        )
    ...
    def _process_multiline_elements(
        self, text: str
    ) -> Tuple[str, List[Tuple[int, int, Dict[str, Any]]]]:
        """
    ...
    def _is_empty_paragraph(self, block: Dict[str, Any]) -> bool:
        ...
        """
        return block.get("type") == "paragraph" and (
            not block.get("paragraph", {}).get("rich_text")
            or len(block.get("paragraph", {}).get("rich_text", [])) == 0
        )
