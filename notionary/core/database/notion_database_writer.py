...
class NotionRelationHandler(LoggingMixin):
    ...
    async def find_relation_by_title(self, database_id: str, relation_prop_name: str, title: str) -> Optional[str]:
        ...
        """
        target_db_id = await self._db_schema.get_relation_database_id(relation_prop_name)
        if not target_db_id:
    ...
    async def _get_title_properties(self, database_id: str, title: str) -> Optional[Dict[str, NotionFormattedValue]]:
        """
...
class DatabaseWritter(LoggingMixin):
    ...
    def __init__(self, client: NotionClient, db_schema: Optional[NotionDatabaseSchema] = None) -> None:
        """
    ...
    async def _ensure_schema_for_database(self, database_id: str) -> bool:
        ...
        self._relation_handler = NotionRelationHandler(self._client, self._active_schema)
    ...
    async def create_page(self, 
                         database_id: str, 
                         properties: Dict[str, Any], 
                         relations: Optional[Dict[str, Union[str, List[str]]]] = None) -> Optional[PageCreationResponse]:
        """
    ...
    async def update_page(self, 
                         ...
                         relations: Optional[Dict[str, Union[str, List[str]]]] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _format_properties(self, database_id: str, properties: Dict[str, Any]) -> Optional[Dict[str, NotionFormattedValue]]:
        """
        ...
        if not property_types:
            self.logger.error("Could not get property types for database %s", database_id)
            return None
    ...
    async def _process_relations(self, database_id: str, relations: Dict[str, Union[str, List[str]]]) -> Dict[str, NotionRelationProperty]:
        """
        ...
        property_types = await self._active_schema.get_property_types() if self._active_schema else {}
    ...
    async def _process_single_relation(
        ...
        if prop_type != "relation":
            self.logger.warning("Property '%s' is not a relation (type: %s)", prop_name, prop_type)
            return None
    ...
    async def _get_relation_ids(self, database_id: str, prop_name: str, titles: List[str]) -> List[str]:
        """
