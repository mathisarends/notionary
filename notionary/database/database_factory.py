...
class NotionDatabaseFactory:
    ...
    async def _convert_rich_text_to_markdown(self, rich_text_objects: list) -> str:
        from notionary.blocks.rich_text.rich_text_markdown_converter import RichTextToMarkdownConverter

        rich_text_markdown_converter = RichTextToMarkdownConverter()
        return await rich_text_markdown_converter.to_markdown(rich_text_objects)

    async def _extract_description(self, response: NotionDatabaseDto) -> str:
        return await self._convert_rich_text_to_markdown(response.description)
    ...
    async def _extract_data_sources(self, response: NotionDatabaseDto) -> list[NotionDataSource]:
        ...
        return await asyncio.gather(*(NotionDataSource.from_id(data_source_id) for data_source_id in data_source_ids))
