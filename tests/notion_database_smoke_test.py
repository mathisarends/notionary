...
class TestNotionDatabaseSmoke:
    ...
    async def test_database_creation_from_name(self):
        ...
        database = await NotionDatabase.from_database_name(self.DATABASE_NAME, self.token)
        ...
        print(f"✅ Found: \"{self.DATABASE_NAME}\" 📊 → Database ID: {database.database_id}")
        return database
    ...
    async def test_database_from_id(self):
        ...
        database_from_id = await NotionDatabase.from_database_id(TestNotionDatabaseSmoke.database_id, self.token)
    ...
    async def test_sort_pages(self):
        ...
        # Get sorted pages
        sorted_pages = await database.get_pages(
            limit=5,
            sorts=sorts
        )
    ...
    async def test_archive_page(self):
        ...
        print(f"✅ Successfully archived page with ID: {TestNotionDatabaseSmoke.test_page_id}")
    ...
    async def _get_test_database(self):
        ...
        if hasattr(TestNotionDatabaseSmoke, 'database_id'):
            return await NotionDatabase.from_database_id(TestNotionDatabaseSmoke.database_id, self.token)
        else:
            database = await self.test_database_creation_from_name()
            TestNotionDatabaseSmoke.database_id = database.database_id
            return database
...
async def run_all_tests():
    ...
    if hasattr(TestNotionDatabaseSmoke, 'database_id'):
        database = await NotionDatabase.from_database_id(
            TestNotionDatabaseSmoke.database_id, 
            test_instance.token
        )
