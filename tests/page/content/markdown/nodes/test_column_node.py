...
def test_column_without_width_ratio(
    syntax_registry: SyntaxDefinitionRegistry, column_delimiter: str
) -> None:
...
def test_column_with_width_ratio(
    syntax_registry: SyntaxDefinitionRegistry, column_delimiter: str
) -> None:
...
def test_column_with_children(
    syntax_registry: SyntaxDefinitionRegistry, column_delimiter: str, indent: str
) -> None:
    child = ParagraphMarkdownNode(
        text="Column content", syntax_registry=syntax_registry
    )
...
def test_column_list_without_columns(
    syntax_registry: SyntaxDefinitionRegistry, column_list_delimiter: str
) -> None:
...
def test_column_list_with_columns(
    ...
    column_list = ColumnListMarkdownNode(
        columns=[column1, column2], syntax_registry=syntax_registry
    )
