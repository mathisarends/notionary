...
def test_callout_without_emoji(
    syntax_registry: SyntaxDefinitionRegistry, callout_delimiter: str
) -> None:
    callout = CalloutMarkdownNode(
        text="This is important", syntax_registry=syntax_registry
    )
...
def test_callout_with_emoji(
    ...
    callout = CalloutMarkdownNode(
        text="Warning!", emoji="⚠️", syntax_registry=syntax_registry
    )
...
def test_callout_with_single_child(
    ...
    child = ParagraphMarkdownNode(
        text="Important details", syntax_registry=syntax_registry
    )
    callout = CalloutMarkdownNode(
        text="Main message", children=[child], syntax_registry=syntax_registry
    )
...
def test_callout_with_multiple_children(
    ...
    child1 = ParagraphMarkdownNode(
        text="Important details", syntax_registry=syntax_registry
    )
    child2 = ParagraphMarkdownNode(
        text="Additional information", syntax_registry=syntax_registry
    )
    callout = CalloutMarkdownNode(
        text="Main callout message",
        emoji="💡",
...
def test_callout_with_empty_children_list(
    syntax_registry: SyntaxDefinitionRegistry, callout_delimiter: str
) -> None:
    callout = CalloutMarkdownNode(
        text="Test", children=[], syntax_registry=syntax_registry
    )
...
def test_callout_with_nested_callouts(
    ...
    nested_callout = CalloutMarkdownNode(
        text="Nested warning", emoji="⚠️", syntax_registry=syntax_registry
    )
    parent_callout = CalloutMarkdownNode(
        text="Parent callout",
        emoji="📢",
        children=[nested_callout],
        syntax_registry=syntax_registry,
    )
