...
@pytest.fixture
def processor(
    syntax_registry: SyntaxDefinitionRegistry, markdown_grammar: MarkdownGrammar
) -> ColumnSyntaxPreProcessor:
...
def test_no_columns_block_should_pass(
    processor: ColumnSyntaxPreProcessor, heading_delimiter: str
) -> None:
...
def test_columns_with_zero_columns_should_fail(
    processor: ColumnSyntaxPreProcessor, column_list_delimiter: str
) -> None:
    invalid_markdown = dedent(
        f"""
        {column_list_delimiter}
