...
@pytest.fixture
def pdf_renderer(mock_rich_text_markdown_converter: RichTextToMarkdownConverter) -> PdfRenderer:
    return PdfRenderer(rich_text_markdown_converter=mock_rich_text_markdown_converter)


@pytest.mark.asyncio
async def test_pdf_block_should_be_handled(pdf_renderer: PdfRenderer, mock_block: Block) -> None:
...
@pytest.mark.asyncio
async def test_non_pdf_block_should_not_be_handled(pdf_renderer: PdfRenderer, mock_block: Block) -> None:
...
async def test_pdf_with_caption_should_include_caption_in_markdown(
    ...
    mock_rich_text_markdown_converter.to_markdown = AsyncMock(return_value="Important document")

    pdf_data = _create_pdf_data_with_caption("https://example.com/document.pdf", caption_rich_text)
    block = _create_pdf_block(pdf_data)
    render_context.block = block
