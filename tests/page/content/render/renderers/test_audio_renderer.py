...
def _create_audio_block(audio_data: AudioData | None) -> AudioBlock:
    block = Mock(spec=AudioBlock)
    block.type = BlockType.AUDIO
    block.audio = audio_data
    return block
...
@pytest.mark.asyncio
async def test_audio_block_should_be_handled(audio_renderer: AudioRenderer, mock_block: Block) -> None:
...
async def test_audio_with_external_url_should_render_markdown_audio_link(
    ...
    audio_data = _create_external_audio_data("https://example.com/audio.mp3")
...
async def test_audio_with_notion_hosted_file_should_render_markdown_audio_link(
    ...
    audio_data = _create_notion_audio_data("https://notion.so/file.mp3", "2025-01-01")
    block = _create_audio_block(audio_data)
    render_context.block = block
...
async def test_audio_with_caption_should_include_caption_in_markdown(
    ...
    audio_data = _create_external_audio_data_with_caption("https://example.com/audio.mp3", caption_rich_text)
    block = _create_audio_block(audio_data)
    render_context.block = block
