...
async def test_validate_passes_when_file_size_within_limit(mock_bot_user: Mock) -> None:
    validator = FileUploadLimitValidator("small_file.pdf", file_size_bytes=5 * 1024 * 1024)
...
async def test_validate_passes_when_file_size_equals_limit(mock_bot_user: Mock) -> None:
    validator = FileUploadLimitValidator("exact_size.pdf", file_size_bytes=20 * 1024 * 1024)
...
async def test_validate_fails_when_file_size_exceeds_limit(mock_bot_user: Mock) -> None:
    validator = FileUploadLimitValidator("large_file.pdf", file_size_bytes=25 * 1024 * 1024)
...
@pytest.mark.asyncio
async def test_validate_fails_when_file_exceeds_small_limit(mock_bot_user_small_limit: Mock) -> None:
...
async def test_validate_extracts_filename_from_path(mock_bot_user: Mock) -> None:
    validator = FileUploadLimitValidator(Path("/some/path/document.pdf"), file_size_bytes=1024)
