...
def mock_extract_text():
    ...
    with patch("notionary.blocks.shared.text_inline_formatter.TextInlineFormatter.extract_text_with_formatting") as mock:
...
def test_roundtrip_conversion(mock_extract_text):
    ...
    for original_markdown in test_cases:
        ...
        assert notion_block is not None, f"Failed to convert {original_markdown} to Notion"
...
@pytest.mark.parametrize("markdown,expected_emoji", [
...
def test_specific_notion_structures(mock_extract_text, simple_callout_block, warning_callout_block):
...
def test_markdown_to_notion_detailed_structure():
    ...
    for case in test_cases:
        ...
        for key, expected_value in case["expected"]["callout"].items():
            if key != "rich_text":
                assert result["callout"][key] == expected_value, \
