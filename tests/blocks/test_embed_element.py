...
from notionary.blocks.file.file_element_models import ExternalFile, NotionHostedFile, FileUploadFile
...
def test_match_markdown_valid():
    ...
    assert EmbedElement.match_markdown('[embed](https://youtube.com/watch?v=123 "Video")')
...
def test_match_markdown_invalid():
    ...
    assert not EmbedElement.match_markdown("[image](https://example.com)")  # Wrong prefix
    assert not EmbedElement.match_markdown("[embed](not-a-url)")  # Invalid URL
    assert not EmbedElement.match_markdown("[embed]()")  # Empty URL
    assert not EmbedElement.match_markdown("[embed](ftp://example.com)")  # Non-http protocol
