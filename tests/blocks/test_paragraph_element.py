...
async def test_match_markdown_valid():
    ...
    assert (
        await ParagraphElement.markdown_to_notion("Text with **bold** formatting") is not None
    )
    ...
    assert (
        await ParagraphElement.markdown_to_notion("Text with trailing spaces  ") is not None
    )
