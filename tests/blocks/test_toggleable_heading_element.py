...
def test_notion_to_markdown_basic():
    block = {
        ...
        "heading_2": {
            "is_toggleable": True,
            "rich_text": [
                {"type": "text", "plain_text": "Ein Titel", "text": {"content": "Ein Titel"}}
            ],
            "color": "default",
            "children": []
        }
...
def test_process_nested_content_callback_usage():
    ...
    def dummy_processor(text):
        return [{"type": "paragraph", "paragraph": {"rich_text": [{"type": "text", "plain_text": text, "text": {"content": text}}]}}]
    ...
    assert heading["children"][0]["paragraph"]["rich_text"][0]["plain_text"] == "Dies ist Child"
