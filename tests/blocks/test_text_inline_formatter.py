...
class TestTextInlineFormatterBasics:
    ...
    async def test_underline_formatting(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting("__underlined__")
        assert len(result) == 1
    ...
    async def test_strikethrough_formatting(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting("~~crossed out~~")
        assert len(result) == 1
    ...
    async def test_code_formatting(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting("`code snippet`")
        assert len(result) == 1
...
class TestTextInlineFormatterLinks:
    ...
    @pytest.mark.asyncio
    async def test_link_variations(self, markdown, expected_text, expected_url):
...
class TestTextInlineFormatterEquations:
    ...
    @pytest.mark.asyncio
    async def test_inline_equations(self, markdown, expected_expression):
        """Test inline equation parsing."""
        result = await TextInlineFormatter.parse_inline_formatting(markdown)
        assert len(result) == 1
...
class TestTextInlineFormatterColors:
    ...
    @pytest.mark.asyncio
    async def test_valid_colors(self, markdown, expected_text, expected_color):
    ...
    async def test_invalid_color_fallback(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting("(invalid:text)")
        assert len(result) == 1
...
class TestTextInlineFormatterMentions:
    ...
    async def test_page_mention(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting(f"@page[{page_id}]")
        assert len(result) == 1
    ...
    async def test_user_mention(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting(f"@user[{user_id}]")
        assert len(result) == 1
        assert result[0].type == RichTextType.MENTION
...
class TestTextInlineFormatterComplex:
    ...
    @pytest.mark.asyncio
    async def test_mixed_formatting(self):
    ...
    @pytest.mark.asyncio
    async def test_equation_with_text(self):
...
class TestTextInlineFormatterRoundtrip:
    ...
    @pytest.mark.asyncio
    async def test_equation_roundtrip(self):
...
class TestTextInlineFormatterEdgeCases:
    ...
    async def test_plain_text_only(self):
        ...
        result = await TextInlineFormatter.parse_inline_formatting("just plain text")
        assert len(result) == 1
    ...
    @pytest.mark.asyncio
    async def test_incomplete_patterns(self):
        ...
        for pattern in incomplete_patterns:
            result = await TextInlineFormatter.parse_inline_formatting(pattern)
    ...
    async def test_unicode_and_special_chars(self):
        ...
        for case in test_cases:
            result = await TextInlineFormatter.parse_inline_formatting(case)
            # Should parse without errors
            assert len(result) >= 1
            # Roundtrip should work
            back = await TextInlineFormatter.extract_text_with_formatting(result)
            assert back == case
