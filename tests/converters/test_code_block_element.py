...
from notionary.blocks import CodeBlockElement
...
class TestCodeBlockElement(unittest.TestCase):
    ...
    def test_match_markdown(self):
        ...
        self.assertTrue(
            CodeBlockElement.match_markdown("```python\nprint('Hello')\n```")
        )
        self.assertTrue(
            CodeBlockElement.match_markdown("```js\nconsole.log('hi');\n```")
        )
    ...
    def test_markdown_to_notion_no_language(self):
        ...
        self.assertEqual(
            block["code"]["rich_text"][0]["text"]["content"], "just some code"
        )
    ...
    def test_find_matches(self):
        ...
        self.assertIn(
            "print('Hello')", matches[0][2]["code"]["rich_text"][0]["text"]["content"]
        )
