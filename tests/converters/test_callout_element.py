...
from notionary.converters.elements.callouts import CalloutElement
...
class TestCalloutElement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.extract_text_patcher = patch('notionary.converters.elements.text_formatting.extract_text_with_formatting')
    ...
    def test_match_markdown(self):
        ...
        self.assertTrue(CalloutElement.match_markdown("!> {blue_background} [🔥] Ein Callout mit Farbe und Emoji"))
    ...
    def test_markdown_to_notion_and_back(self):
        ...
        for markdown in test_cases:
            ...
            self.assertIsNotNone(notion_block, f"Konnte {markdown} nicht zu Notion konvertieren")
            
            # Konvertiere Notion zurück zu Markdown
            markdown_result = CalloutElement.notion_to_markdown(notion_block)
            self.assertIsNotNone(markdown_result, f"Konnte Notion-Block nicht zurück zu Markdown konvertieren")
            ...
            self.assertIn("!>", markdown_result, "Markdown-Ergebnis enthält nicht das Callout-Präfix")
        ...
        for case in test_cases:
            ...
            for key, expected_value in case["expected_notion"]["callout"].items():
                if key != "rich_text":  # rich_text wird separat verarbeitet
                    self.assertEqual(notion_result["callout"][key], expected_value,
    ...
    def test_notion_to_markdown_examples(self):
        ...
        test_cases = [
            {
                "input": {
                    "type": "callout",
                    "callout": {
                        "rich_text": [{"type": "text", "text": {"content": "Einfacher Callout"}}],
                        "icon": {"type": "emoji", "emoji": "💡"},
                        "color": "gray_background"
                    }
                },
            ...
            },
            {
                "input": {
                    "type": "callout",
                    "callout": {
                        "rich_text": [{"type": "text", "text": {"content": "Callout mit Farbe"}}],
                        "icon": {"type": "emoji", "emoji": "💡"},
                        "color": "blue_background"
                    }
                },
            ...
            },
            {
                "input": {
                    "type": "callout",
                    "callout": {
                        "rich_text": [{"type": "text", "text": {"content": "Callout mit Emoji"}}],
                        "icon": {"type": "emoji", "emoji": "🔥"},
                        "color": "gray_background"
                    }
                },
        ...
        for i, case in enumerate(test_cases):
            ...
            self.assertIsNotNone(result, f"notion_to_markdown gab None für Testfall {i+1} zurück")
            
            # Überprüfen grundlegender Anforderungen
            self.assertIn("!>", result, f"Ergebnis enthält nicht das Callout-Präfix für Testfall {i+1}")
            
            # Überprüfen des Emojis
            expected_emoji = case["expected_emoji"]
            if expected_emoji:
                self.assertIn(expected_emoji, result, f"Emoji {expected_emoji} fehlt im Ergebnis für Testfall {i+1}")
            
            # Überprüfen der Farbe
            expected_color = case["expected_color"]
            if expected_color:
                self.assertIn(expected_color, result, f"Farbe {expected_color} fehlt im Ergebnis für Testfall {i+1}")
            
            # Überprüfen des Inhalts
            self.assertIn(expected_content, result, f"Inhalt '{expected_content}' fehlt im Ergebnis für Testfall {i+1}")
